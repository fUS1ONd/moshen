...
//
// Сбрасывается только окно (ResetResult), но не идущий вызов: залп приходит к
// одному провайдеру с нескольких сторон сразу (общая карта плюс каждая группа
// с use:), и полный Reset заставлял бы каждого запустить свою пробу по всем
// нодам. Присоединиться к идущей проверке достаточно: если она стартовала ещё
// на прежнем пути, её результат уточнит второй залп гейта готовности, который
// уходит уже по поднявшейся сети.
func (hc *HealthCheck) forceCheck() {
	hc.singleDo.ResetResult()
	hc.check()
}
